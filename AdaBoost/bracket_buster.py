...
class BracketBuster():
	def __init__(self):
		...
		self.bracket_seeds = pickle.load(open("pickled_files/bracket_seeds.p", 'rb'))
		self.bracket_tuples = pickle.load(open("pickled_files/bracket_tuples.p", 'rb'))
	...
	def adaboost_predict(self, season, team1, team2):
		# Initialize external model
		boost = AdaBoost()
		fname = "pickled_files/adaboost.p"
		#fname = "pickled_files/adaboost_" + season + ".p"
		boost = boost.load(fname)

		# Load feature vecs for each team
		#print("season: %s" % season)
		try:
			x_vec = list(self.feature_vec[season][team1]) + list(self.feature_vec[season][team2])
			return boost.predict([x_vec])
		except:
