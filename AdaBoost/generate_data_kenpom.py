...
# Create a feature vec from the kenpom data
feature_vec = {}
for x in range(len(kenpom_teams)):
    min_dist = 9999
    min_team = 'None'
    # Calculate the levinshtein distance of the name and the database of team names to find the Team ID #
    for team in teams.values():
        distance = editdistance.eval(kenpom_teams[x], team)
        if distance <= min_dist:
            min_dist = distance
            min_team = team
    # Ensure a team match was found and only take data from after year 2002
    if min_team != None and int(kenpom_years[x]) > 2002:
        if feature_vec.get(str(kenpom_years[x])) is None:
            feature_vec[str(kenpom_years[x])] = {}
        for k, v in teams.items():
            if v == min_team:
                min_team_id = k
        # Create a new entry in the feature vec
        feature_vec[str(kenpom_years[x])][min_team_id] = np.asarray(new_kenpom_data[x])
        # Troubleshoot
        # print('Year: ', str(kenpom_years[x]))
        # print('Team: ', min_team_id)
        # print(feature_vec[str(kenpom_years[x])][min_team_id])

# Populate the train_x and train_y arrays
train_x = {}
train_y = {}
for i, game in enumerate(season_tuples):
    if not feature_vec.get(game[0]).get(game[1]) is None and not feature_vec.get(game[0]).get(game[2]) is None:
        train_x[i] = (list(feature_vec[game[0]][game[1]]))
        train_x[i].extend(list(feature_vec[game[0]][game[2]]))
        train_y[i] = game[3]
