        ...
        for lines in fi:
            ...
            if random.random() < 0.5:
                if season not in results:
                    results[season] = []
                results[season].append((w_team, l_team, 1))
            else:
                if season not in results:
                    results[season] = []
                results[season].append((l_team, w_team, 0))

    return results



def training_tuples_game_id(filename, seedfile):
    ...
    with open(filename, "rb") as fi:
        ...
        with open(seedfile, 'rb') as fi2:
            for line in fi2:
                l2 = line.decode('utf-8').rstrip('\r\n').split(',')
                season2 = l2[0]
                seed = l2[1]
                team_id = l2[2]
                if team_id == w_team:
...
def parse_seeds():
    ...
    with open(tourney_seeds, 'rb') as fi:
        ...
        for lines in fi:
            ...
            #print("Y: %s\t| S: %s\t| T: %s" % (year, seed, team))
            if year not in bracket_seeds:
                bracket_seeds[year] = {}
...
feature_vec = pickle.load(open(feature_file, 'rb'))
#'''
...
results = pickle.load(open("season_tuples.p", 'rb'))
...
seed_file = "data/TourneySeeds.csv"
results = training_tuples(bracket_file, seed_file)
bracket_seeds = parse_seeds()

#results = pickle.load(open("bracket_tuples.p", 'rb'))
#bracket_seeds = pickle.load(open("bracket_seeds.p", 'rb'))

pickle.dump(results, open("bracket_tuples.p", "wb"))
pickle.dump(bracket_seeds, open("bracket_seeds.p", "wb"))
#print(results)
